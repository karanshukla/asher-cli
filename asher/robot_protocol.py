"""Structural protocol for the pylitterbot robot interface used by Asher."""

from __future__ import annotations

from collections.abc import Callable
from datetime import datetime
from typing import Any, Protocol

from pylitterbot.activity import Activity, Insight
from pylitterbot.enums import BrightnessLevel, LitterBoxStatus, NightLightMode


class RobotProtocol(Protocol):
    # ── identity ────────────────────────────────────────────────────────────
    @property
    def name(self) -> str: ...
    @property
    def serial(self) -> str: ...
    @property
    def model(self) -> str: ...

    # ── state ───────────────────────────────────────────────────────────────
    @property
    def is_online(self) -> bool: ...
    @property
    def status(self) -> LitterBoxStatus: ...
    @property
    def waste_drawer_level(self) -> float: ...
    @property
    def last_seen(self) -> datetime | None: ...
    @property
    def sleep_mode_enabled(self) -> bool: ...
    @property
    def panel_lock_enabled(self) -> bool: ...
    @property
    def night_light_mode_enabled(self) -> bool: ...

    # ── LR4+ optional state (absent on LR3 — access via getattr) ────────────
    @property
    def night_light_mode(self) -> NightLightMode | None: ...
    @property
    def night_light_brightness(self) -> int: ...
    @property
    def panel_brightness(self) -> BrightnessLevel | None: ...
    @property
    def litter_level(self) -> float: ...
    @property
    def pet_weight(self) -> float: ...
    @property
    def power_status(self) -> str: ...
    @property
    def clean_cycle_wait_time_minutes(self) -> int: ...

    # VALID_WAIT_TIMES is a class attribute on the concrete robot classes.
    VALID_WAIT_TIMES: list[int]

    # ── methods ─────────────────────────────────────────────────────────────
    async def refresh(self) -> None: ...
    async def start_cleaning(self) -> bool: ...
    async def set_panel_lockout(self, value: bool) -> bool: ...
    async def set_sleep_mode(self, value: bool) -> bool: ...
    async def set_night_light_mode(self, mode: NightLightMode) -> bool: ...
    async def set_night_light_brightness(self, brightness: int) -> bool: ...
    # LR4/LR5-only setter (absent on LR3 — the adapter gates this).
    async def set_panel_brightness(self, brightness: BrightnessLevel) -> bool: ...
    async def get_activity_history(self, limit: int) -> list[Activity]: ...
    async def set_wait_time(self, wait_time: int) -> bool: ...
    async def set_power_status(self, value: bool) -> bool: ...
    async def set_name(self, name: str) -> bool: ...
    async def get_insight(self, days: int = 30, timezone_offset: int | None = None) -> Insight: ...
    # LR5-only setters (absent on LR3/LR4 — the adapter gates these).
    async def set_privacy_mode(self, value: bool) -> bool: ...
    async def set_volume(self, volume: int) -> bool: ...
    async def set_camera_audio(self, value: bool) -> bool: ...
    async def reset_waste_drawer(self) -> bool: ...
    async def subscribe(self) -> None: ...
    async def unsubscribe(self) -> None: ...
    def on(self, event_name: str, callback: Callable[..., Any]) -> Callable[..., Any]: ...
